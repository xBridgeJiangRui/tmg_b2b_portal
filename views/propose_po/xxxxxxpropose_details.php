      ...
      <div class="box box-default">
        ...
        <div class="col-md-12">
            ...
            <div class="row">
              ...
              <div class="col-md-2">
                <input id="delivery_date" name="delivery_date" type="datetime" autocomplete="off" placeholder="yyyy-mm-dd" class="form-control pull-right" disabled>
              </div>
              ...
              <div class="col-md-5">
                <select id="outright_location" name="outright_location[]" class="form-control select2" required disabled>
                  ...
                  <?php foreach($location as $row){ ?>
                    <option value="<?php echo addslashes($row->BRANCH_CODE) ?>" <?php echo (isset($_GET['id']) && $row->branch_description == $location_list[0]) ? 'selected' : '' ; ?>>
            ...
            method:"POST",
              ...
              success:function(data)
              {
                ...
                if (json.count == 1) {
                  ...
                  // console.log(moment(results[0].delivery_date).format("YYYY-MM-DD, ddd"));
            ...
            "ajax": {
              ...
              columns: [
                ...
                {"data" : "order_lot/ctn_qty", render:function( data, type, row ){
                  ...
                  return order_lot_c + "</br><span class='ctn_qty_c' id='ctn_qty_c'>" + ctn_qty_c + "</span>";
                ...
                {"data" : "proposed_bulk_qty", render:function( data, type, row ){
                  ...
                  if(!isNaN(ctn_qty_c) && ctn_qty_c > 0){
                    ...
                    if(parseInt(packsize_c) == 0 || parseInt(packsize_c) == 1){
                      reminder_qty = ctn_qty;
                      ctn_qty = 0;
                    }
                ...
                {"data" : "proposed_qty", render:function( data, type, row ){
                  ...
                  if(backendcheck_status == false || (max_packsize == true && ctn_qty_c != 1)){
                    var input_behaviour = "disabled";
                  }else{
                    var input_behaviour = "";
                  }
                ...
                {"data" : "total_qty", render:function( data, type, row ){
                  ...
                  if(!isNaN(store_qty) && store_qty != '' && store_qty > 0){
                    ...
                    if(packsize_c == 0 || packsize_c == 1){
                      reminder_qty = ctn_qty;
                      ctn_qty = 0;
                    }

                    element += "</br><span style='float: left; font-weight: bold;' class='append_supplier_qty_desc' id='append_supplier_qty_desc'>[" + ctn_qty + " " + umbulk + " " + reminder_qty + " " + um + "]</span>";
                  }else{
        ...
        // $('#loader_div').addClass('hidden');
          ...
          $('#table3').on('input', '.to_update_cost', function() {
            ...
            if(!isNaN(ctn_qty_c) && ctn_qty_c != 0){
              ...
              var ctn_qty = Math.floor(formattedQuantity / ctn_qty_c);
              var total_ctn_value = Number(parseFloat(ctn_qty_c*ctn_qty)).toFixed(2);
              var reminder_qty = Number(parseFloat(formattedQuantity - total_ctn_value));
