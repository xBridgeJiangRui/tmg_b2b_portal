	...
	public function send() {
		...
		if(!empty($this->cc)){
			...
			foreach ($this->cc as $cc_email) {
				$implode[] = $cc_email['Email'];
				array_push($recipient_list, $cc_email['Email']);
			}

			$cc_list = implode(",", $implode);
		}else{
			$cc_list = '';
		}
		...
		if ($this->protocol == 'mail') {
			...
			if (!$handle) {
				...
				if (substr($reply, 0, 3) != 250) {
					return 'Error: EHLO not accepted from server!';
				}
				...
				if (substr($this->smtp_hostname, 0, 3) == 'tls') {
					...
					if (substr($reply, 0, 3) != 220) {
						return 'Error: STARTTLS not accepted from server!';
					}
				...
				if (!empty($this->smtp_username)  && !empty($this->smtp_password)) {
					...
					if (substr($reply, 0, 3) != 334) {
						return 'Error: AUTH LOGIN not accepted from server!';
					}
					...
					if (substr($reply, 0, 3) != 334) {
						return 'Error: Username not accepted from server!';
					}
					...
					if (substr($reply, 0, 3) != 235) {
						return 'Error: Password not accepted from server!';
					}
				} else {
					...
					if (substr($reply, 0, 3) != 250) {
						return 'Error: HELO not accepted from server!';
					}
				}
				...
				if (substr($reply, 0, 3) != 250) {
					return 'Error: MAIL FROM not accepted from server!';
				}

				array_push($recipient_list, $this->to);
				...
				foreach ($recipient_list as $recipient) {
					...
					if ((substr($reply, 0, 3) != 250) && (substr($reply, 0, 3) != 251)) {
						return 'Error: RCPT TO not accepted from server!';
					}
				}
				...
				if (substr($reply, 0, 3) != 354) {
					return 'Error: DATA not accepted from server!';
				}
				...
				if (substr($reply, 0, 3) != 221) {
					return 'Error: QUIT not accepted from server!';
				}
